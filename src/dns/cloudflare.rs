...
impl CloudflareBackend {
    pub fn new(token: String, zone_id: String, domain: String) -> Self {
...
impl DnsBackend for CloudflareBackend {
    async fn create_record(&self, name: &str, content: &str, ttl: u32) -> Result<String, DnsError> {
        // Cloudflare TXT record max is ~2048 bytes
        if content.len() > 2048 {
            return Err(DnsError::RecordTooLarge {
                size: content.len(),
                max: 2048,
            });
        }
        ...
        let cf_resp: CfResponse<CfRecord> = resp
            .json()
            .await
    ...
    async fn get_records(&self, name: &str) -> Result<Vec<TxtRecord>, DnsError> {
        ...
        let url = format!(
            "{}&type=TXT&name={}",
            self.api_url("/dns_records?per_page=100"),
            name
        );
        ...
        let cf_resp: CfResponse<Vec<CfRecord>> = resp
            .json()
            .await
            .map_err(|e| DnsError::ApiError(e.to_string()))?;
    ...
    async fn list_records(&self, prefix: &str) -> Result<Vec<TxtRecord>, DnsError> {
        ...
        loop {
            let url = format!(
                "{}&type=TXT&per_page=100&page={}",
                self.api_url("/dns_records?"),
                page
            );
